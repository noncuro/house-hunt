...
export function SignIn({ onSignedIn }: { onSignedIn: (state: AuthState) => void }) {
  ...
  async function signIn() {
    setBusy('signing-in');
    const reply = await send({ type: 'auth:sign-in', email: address, password });
    setBusy(null);
    if (!reply.ok) return setOutcome({ kind: 'failed', message: reply.error });
    if (reply.data.status === 'signed-in') return onSignedIn(reply.data.state);
    setOutcome(fromSignIn(reply.data));
  }
  ...
  async function redeem() {
    setBusy('redeeming');
    const reply = await send({ type: 'auth:redeem', email: address, password, code: code.trim() });
    if (!reply.ok) {
      setBusy(null);
      return setOutcome({ kind: 'failed', message: reply.error });
    }
    if (reply.data.status !== 'redeemed') {
      setBusy(null);
      return setOutcome(fromRedeem(reply.data));
    }
    setBusy('signing-in');
    const session = await send({ type: 'auth:sign-in', email: address, password });
    setBusy(null);
    if (!session.ok) return setOutcome({ kind: 'failed', message: session.error });
    if (session.data.status === 'signed-in') return onSignedIn(session.data.state);
    ...
    <div className="settings signin">
      <section className="setting">
        ...
        <p className="dim">
          {mode === 'sign-in' ? (
            <>
              This extension is invite-only. Sign in with the address you were invited on and the
              password you chose. There is nothing to click in an email — nothing here sends any.
            </>
          ) : (
            <>
