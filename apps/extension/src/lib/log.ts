...
// `logInfo` / `logWarn` / `logError` are re-exported from core rather than defined here. Core
// emits through a sink the host registers, and `configureCore()` registers this module's `log` as
// that sink — so a line written from shared code and a line written from the extension land in the
// same ring buffer and read the same way in Settings. Two sets of helpers writing to one buffer
// would be a coin flip over which one a caller reached for.
export { logInfo, logWarn, logError } from '@house-hunt/core';
