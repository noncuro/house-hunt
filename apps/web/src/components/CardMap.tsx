...
/** A map of one flat, on the card itself, behind a button.
 *
 *  `Hub.tsx` already says "0.4 mi NE of Angel", which is the sentence — this is the picture behind
 *  it: the flat, the hubs it sits between, and the streets in between them. Reading a shortlist is
 *  mostly deciding whether somewhere is anywhere you'd live, and the full Map view answers that one
 *  flat at a time with a view change in each direction.
 *
 *  Behind a button rather than always drawn, because a page of cards is fifty maps and fifty maps
 *  is several hundred tile requests for a scroll past places you have already decided about. Asking
 *  is also the honest default: you want the streets for one flat at a time.
 *
 *  Same tiles, same colours and the same postcode-derived position as `screens/Map.tsx` — a
 *  thumbnail that placed a flat somewhere the map above doesn't would be worse than no thumbnail. */
