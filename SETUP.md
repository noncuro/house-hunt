...
3. Repoint `travel_functions_url` at the site.
4. Confirm a run works (the queries below), then drop what nothing reads any more —
   `delete from vault.secrets where name in ('travel_service_role_key', 'travel_publishable_key');`.
   Nothing removes them for you: a migration that deletes a secret it did not create destroys a
   working deployment if it is ever re-run mid-rollout.
