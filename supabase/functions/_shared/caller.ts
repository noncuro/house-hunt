 ...
 *
 *  Every function here used to be reachable by anyone holding the URL, which was defensible while
 *  the bundle was on two laptops and the publishable key *was* the shared secret. It is not
 *  defensible once the bundle is on other people's machines and every call is a claim on the
 *  owner's OpenAI key, so the caller is resolved from a real session before anything else happens
 *  (design D10).
 *
 *  The token is verified by asking GoTrue rather than by decoding it here. That costs one round
 *  trip and buys three things a local decode does not: an expired token is rejected by the
 *  authority on when it expired, a deleted or banned user stops being a caller immediately, and the
 *  check does not quietly depend on the platform's `verify_jwt` still being switched on. The
 *  publishable key presented as a bearer fails it too, which is the case that matters most — that
 *  key is in every bundle.
 */
import { eq, HttpError, rest, SERVICE_KEY, SUPABASE_URL } from './http.ts';
 ...
 *
 *  Membership is checked here rather than left to RLS because these functions hold the service role
