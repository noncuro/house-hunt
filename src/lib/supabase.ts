 ...
 */
import { accessToken, requireSession, supabase } from './auth';
...
export async function renameProject(projectId: string, name: string): Promise<void> {
  ...
  const { error } = await supabase.from('project').update({ name: trimmed }).eq('id', projectId);
...
export async function headcount(projectId: string): Promise<Headcount> {
  const { data, error } = await supabase.rpc('project_headcount', { p_project_id: projectId });
...
export async function recordProperty(listing: Listing): Promise<void> {
  ...
  const { error } = await supabase.rpc('record_property', {
...
export async function setVerdict(rightmoveId: string, rating: Rating, note: string): Promise<void> {
  const session = await requireSession();
  const projectId = await activeProjectId();
  const { error } = await supabase.from('verdict').upsert(
    {
...
export async function removePlace(id: string): Promise<void> {
  const projectId = await activeProjectId();
  const { error } = await supabase.from('place').delete().eq('id', id).eq('project_id', projectId);
...
export async function cacheTravel(
  ...
  const { error } = await supabase.rpc('cache_travel', {
...
export async function cacheStationWalk(postcode: string, name: string, seconds: number): Promise<void> {
  const { error } = await supabase.rpc('cache_station_walk', {
    p_postcode: postcode,
    p_station_name: name,
    p_seconds: seconds,
  });
...
export async function cacheStationPoint(name: string, station: StationInfo | null): Promise<void> {
  const { error } = await supabase.rpc('cache_station_point', {
    p_name: name,
...
export async function recordSightings(hub: string, cards: SearchCard[]): Promise<void> {
  ...
  const { error } = await supabase.from('search_sighting').upsert(
...
export async function removeHub(id: string): Promise<void> {
  const projectId = await activeProjectId();
  const { error } = await supabase.from('project_hub').delete().eq('id', id).eq('project_id', projectId);
...
export async function locateProperties(): Promise<number> {
  ...
  for (const row of rows) {
    ...
    if (!point) continue;
    const { error: writeError } = await supabase.rpc('set_property_point', {
      p_project_id: projectId,
...
export async function listInvites(projectId?: string): Promise<Invite[]> {
  let query = supabase.from('invite').select(INVITE_COLUMNS).order('created_at', { ascending: false });
...
export async function consumeInvites(): Promise<void> {
  const { error } = await supabase.rpc('consume_invites');
...
export async function redeemInvite(email: string, password: string, code: string): Promise<RedeemResult> {
  ...
  const { data, error } = await supabase.functions.invoke('password', {
...
export async function adminSetPassword(userId: string, password: string): Promise<void> {
  ...
  const { error } = await supabase.functions.invoke('password', {
...
export async function spendSummary(): Promise<SpendSummary> {
  const session = await requireSession();
  const projectId = await activeProjectId();
  const { data, error } = await supabase.rpc('spend_summary', {
    p_project_id: projectId,
...
export async function adminSetUserCap(userId: string, capUsd: number): Promise<void> {
  const { error } = await supabase.rpc('admin_set_user_cap', { p_user_id: userId, p_cap: capUsd });
  fail('changing a personal cap', error);
}

export async function adminSetProjectCap(projectId: string, capUsd: number): Promise<void> {
  const { error } = await supabase.rpc('admin_set_project_cap', { p_project_id: projectId, p_cap: capUsd });
...
export async function adminSetMaxMembers(projectId: string, maxMembers: number): Promise<void> {
  const { error } = await supabase.rpc('admin_set_max_members', { p_project_id: projectId, p_max: maxMembers });
