...
import { stationDistance } from '@/lib/facts';
import { send } from '@/lib/messages';
import { FALLBACK_LINE_COLOUR, LINE_COLOURS, textOn } from '@/lib/tfl';
import './stations.css';
...
export function Stations({
  ...
  useEffect(() => {
    ...
    let live = true;
    void send({ type: 'stations:walk', postcode, stations: names }).then((r) => {
      if (live && r.ok) setWalks(r.data);
    });
    return () => {
      live = false;
    };
