...
import { formatDuration, MapsButton, MODE_ICON, readTravel, Routes, TransitBasis } from '@/components/Journey';
import { Stations } from '@/components/Stations';
import { RatingButtons, VerdictLine } from '@/components/Verdict';
...
export function Detail({
  ...
  // Keyed on the places as well as the postcode. The card stays mounted while you visit Settings,
  // so with the postcode alone, adding a destination left this card rendering that new place from
  // `places` with no cached row to match it — and printing "no route", which is a claim about
  // geography rather than about the fetch never having happened.
  const placeKey = places.map((p) => p.id).join(',');
  useEffect(() => {
    if (!entry.postcode) return;
    let live = true;
    void send({ type: 'travel:get', postcode: entry.postcode }).then((r) => {
      if (live && r.ok) setTravel(r.data);
    });
    return () => {
      live = false;
    };
  }, [entry.postcode, placeKey]);
