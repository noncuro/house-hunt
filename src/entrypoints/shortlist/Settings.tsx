...
import { clearLog, formatLog, readLog } from '@/lib/log';
import { distanceMiles } from '@/lib/hubs';
import { send } from '@/lib/messages';
import type { LocationResult, ProjectHub } from '@/lib/messages';
import { SWEEP_WINDOWS } from '@/lib/sweep';
import type { Place } from '@/lib/types';
import { TRANSIT_BASIS_NOTE } from '@/components/Journey';
...
export function Settings({
  ...
  async function saveName() {
    const result = await send({ type: 'profile:set-name', displayName: name.trim() });
    if (!result.ok) return notify(result.error);
    setPerson(name.trim());
    notify(`You appear as ${name.trim()}.`, 'info');
  }

  async function addPlace() {
    setBusy(true);
    const result = await send({ type: 'places:add', label, postcode: postcode.toUpperCase() });
    setBusy(false);
    if (!result.ok) return notify(result.error);
  ...
  async function removePlace(id: string) {
    const result = await send({ type: 'places:remove', id });
    if (!result.ok) return notify(result.error);
  ...
  async function copyDiagnostics() {
    ...
    if (entries.length === 0) return notify('Nothing logged yet.', 'info');
    await navigator.clipboard.writeText(formatLog(entries));
    notify(`Copied ${entries.length} log lines.`, 'info');
  }
  ...
  return (
    <div className="settings">
      ...
      <section className="setting">
        <h2>Diagnostics</h2>
        <p className="dim">
          What the extension actually did — TfL calls, retries, failures. The background worker's
          own console is wiped whenever Chrome idles it out, so it is kept here instead.
        </p>
        <div className="fields">
          <button onClick={() => void copyDiagnostics()}>Copy log</button>
          <button
            onClick={() => {
              void clearLog();
...
function Neighbourhoods({ notify }: { notify: (text: string, kind?: 'error' | 'info') => void }) {
  ...
  useEffect(() => {
    void (async () => {
      const result = await send({ type: 'hubs:list' });
  ...
  async function add() {
    ...
    const result = await send({ type: 'hubs:add', hub: { name, postcode: where.trim() || undefined } });
    setBusy(false);
    if (!result.ok) return notify(result.error);
    setHubs([...(hubs ?? []), result.data]);
    setName('');
    setWhere('');
  }
  ...
  async function remove(hub: ProjectHub) {
    ...
    const result = await send({ type: 'hubs:remove', id: hub.id });
    if (!result.ok) return notify(result.error);
  ...
  async function resolve(hub: ProjectHub) {
    setBusy(true);
    const result = await send({ type: 'hubs:resolve-location', name: hub.name });
    setBusy(false);
    if (!result.ok) return notify(result.error);
    setLocated({ ...located, [hub.id]: result.data });
    ...
    const saved = await send({
      ...
      id: hub.id,
      patch: {
        locationIdentifier: result.data.locationIdentifier,
        displayLocationIdentifier: result.data.displayLocationIdentifier,
      },
    });
    if (!saved.ok) return notify(saved.error);
    setHubs((hubs ?? []).map((h) => (h.id === hub.id ? saved.data : h)));
  }
  ...
  async function setWindow(hub: ProjectHub, days: number | null) {
    const result = await send({
      type: 'hubs:update',
      id: hub.id,
      patch: { maxDaysSinceAdded: days ?? undefined },
    });
    if (!result.ok) return notify(result.error);
    setHubs((hubs ?? []).map((h) => (h.id === hub.id ? result.data : h)));
  }
