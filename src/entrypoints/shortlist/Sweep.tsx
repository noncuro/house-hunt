...
import { toSweepHub } from '@/lib/hubs';
import type { ProjectHub, ResponseMap } from '@/lib/messages';
import { sweepSearchUrl, sweepWindow, windowLabel } from '@/lib/sweep';

/** Taken off the message contract rather than imported from `lib/supabase`, which a view may not
 *  touch (design D2). */
type HubSweepRow = ResponseMap['sweep:hubs'][number];
...
export function Sweep() {
  ...
  const hubs = useQuery({
    queryKey: ['hubs'],
    queryFn: async () => await ask({ type: 'hubs:list' }),
  });

  const sweeps = useQuery({
    queryKey: ['sweeps'],
    queryFn: async () => await ask({ type: 'sweep:hubs' }),
  });

  const pending = useQuery({
    queryKey: ['pending'],
    queryFn: async () => await ask({ type: 'sweep:pending' }),
...
function HubRow({ hub, sweep }: { hub: ProjectHub; sweep: HubSweepRow | null }) {
