...
import { duplicateIds, enthusiasm, groupOf, sizeOf, type Group } from '@/lib/shortlist';
import type { ShortlistEntry } from '@/lib/supabase';
import type { Place, Rating } from '@/lib/types';
import { HubFact } from '@/components/Hub';
import { Hint } from '@/components/Hint';
import { Flags } from '@/components/Flags';
import { SizeFact } from '@/components/Size';
import { SpendWarning } from '@/components/Spend';
import { RATINGS, ratingOf } from '@/components/ratings';
import { hubsFromProject, type Hub } from '@/lib/hubs';
import { Compare } from './Compare';
import { Detail } from './Detail';
import { Admin } from './Admin';
import { Project, ProjectPicker } from './Project';
import { Settings } from './Settings';
import { SignIn } from './SignIn';
import { ShortlistMap } from './Map';
import type { AuthState, ProjectSummary, SessionUser } from '@/lib/messages';
import {
  ask,
  BackgroundError,
  keys,
...
/** What the page is at all is decided here, and by one question: `auth:state`.
 *
...
function Root() {
  ...
  // The worker itself is unreachable — a different thing from being signed out, and it must not
...
function App({ user, project }: { user: SessionUser; project: ProjectSummary }) {
  ...
  // The neighbourhoods every card places its flat against (design D11). Same key the Sweep view
  // reads, so switching between them costs nothing and the two cannot disagree about which
  // neighbourhoods this house hunt has.
  const hubsQuery = useQuery({ queryKey: ['hubs'], queryFn: () => ask({ type: 'hubs:list' }) });
  ...
  const spendQuery = useQuery({ queryKey: ['spend'], queryFn: () => ask({ type: 'spend:summary' }) });
  ...
  /** Jump from an overview — a map pin, a table row — to the place itself. Nothing to open now
   *  that every card shows everything; it is purely a scroll. */
  ...
  if (shortlist.isError) {
    ...
    // view's error to report: the shell is already re-reading `auth:state` and is about to replace
    // the whole page with the sign-in view or the picker. Printing "sign in to the house hunt
    // extension" here would flash that sentence with no field underneath it to act on.
    const handled =
      shortlist.error instanceof BackgroundError &&
      (shortlist.error.unauthenticated || shortlist.error.noProject);
    return (
      <div className="wrap">
        {handled ? (
          <p className="dim working">Loading…</p>
  ...
  return (
    ...
    <div className={view === 'table' ? 'wrap wrap-wide' : 'wrap'}>
      ...
      {view === 'table' && (
        <Compare entries={entries} places={places} onOpen={(id) => openCard(id)} />
      )}
